...
var changeValue = [];
var changeMonthValue = [];
var changeTotal = 0;

// Change tracked from month 2 (index 1) as no calculation from first month for comparison
for (var i = 1; i < finances.length; i++) {
  ...
  //Push monthly changes into the change array
      changeValue.push(changePerMonth);
  //Push month and year and monthly changes into changeMonth array
      changeMonthValue.push([finances[i][0], changePerMonth]);
...
var averageChange = (changeTotal / (finances.length - 1)).toFixed(2);

console.log('Average Change: $' + averageChange);

//Calculate the greatest monthly increase 
//Find the index ID for the greatest profit in the changeValue array
var increaseIndexID = changeValue.indexOf(Math.max(...changeValue));
//Match the index ID to the changeMonthValue array to get the month and change values
var greatestIncrease = changeMonthValue[increaseIndexID];

//Calculate the greatest monthly decreaes 
//Find the index ID for the greatest loss in the changeValue array
var decreaseIndexID = changeValue.indexOf(Math.min(...changeValue));
//Match the index ID to the changeMonthValue array to get the month and change values
var greatestDecrease = changeMonthValue[decreaseIndexID];
